...
/**
 * Consolidate multiple carousel-hero blocks within the same section
 * into a single multi-slide carousel. Each block in the content
 * represents one slide, but they should render as one carousel.
 */
function consolidateSiblingBlocks(block) {
  const section = block.closest('.section');
  if (!section) return;

  const allHeroBlocks = [...section.querySelectorAll('.carousel-hero')];
  if (allHeroBlocks.length <= 1 || allHeroBlocks[0] !== block) return;

  allHeroBlocks.slice(1).forEach((sibling) => {
    const rows = sibling.querySelectorAll(':scope > div');
    rows.forEach((row) => block.append(row));
    const wrapper = sibling.closest('.carousel-hero-wrapper');
...
export default async function decorate(block) {
  // Skip if this block was removed from DOM by consolidation
  if (!block.isConnected) return;

  // Consolidate sibling carousel-hero blocks into this one
  ...
  if (!isSingleSlide) {
    ...
    // Auto-advance slides every 3 seconds (matching original site)
    ...
    // Only auto-play when the carousel is visible
